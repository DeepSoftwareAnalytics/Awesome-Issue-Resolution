...
from services.paper_list_service import get_paper_list_payload, get_paper_redundancy_report
...
def _apply_paper_category_filter(query, raw_category):
    cats = [c.strip() for c in str(raw_category).split(',') if c.strip()]
    if not cats:
        return query
    normalized_db_category = func.replace(func.coalesce(Paper.category, ''), ' ', '')
    wrapped = literal(',') + normalized_db_category + literal(',')
    conditions = [wrapped.like(f'%,{cat},%') for cat in cats]
...
def get_papers():
    ...
    if category:
        query = _apply_paper_category_filter(query, category)
...
def create_paper():
    ...
    try:
        ...
        paper = Paper(
            ...
            venue=data['venue'],
            category=_normalize_category_value(data['category']),
    ...
    try:
        for key, value in data.items():
            if hasattr(paper, key):
                if key == 'category':
                    value = _normalize_category_value(value)
...
def get_stats_filtered():
    ...
    if category:
        paper_months = _apply_paper_category_filter(paper_months, category)
    if search:
        term = f'%{search}%'
...
@api.route('/agent/paper-list', methods=['GET'])
def agent_paper_list():
    output_format = (request.args.get('format') or 'json').strip().lower()
    payload = get_paper_list_payload(
        session=session,
        ...
        link_priority=request.args.get('link_priority')
    )

    if output_format == 'markdown':
        return payload['context_markdown'], 200, {'Content-Type': 'text/markdown; charset=utf-8'}

    return jsonify(payload)


@api.route('/agent/redundancy-report', methods=['GET'])
def agent_redundancy_report():
    return jsonify(get_paper_redundancy_report(session=session))
...
def import_from_data():
    ...
    try:
        result = subprocess.run(
            [sys.executable, str(config.script('migrate'))],
            capture_output=True,
            text=True,
            timeout=60
        )
...
def render_from_db():
    ...
    try:
        project_root = str(config.BASE_DIR)
        script_path = str(config.script('render_from_db'))
