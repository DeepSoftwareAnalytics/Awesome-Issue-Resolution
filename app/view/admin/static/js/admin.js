...
function getSelectedPapersInOrder(includeUnselected = false) {
    const selectedIds = new Set(selectedPapers.keys());
    const selectedOrdered = currentData.filter(p => selectedIds.has(p.id));
...
function toFlatPaperExportRow(paper) {
    const links = paper.links || {};
    return {
...
function exportSelectedCSV() {
    ...
    const ordered = getSelectedPapersInOrder(true);
    const csv = exportToCSV(ordered.map(toFlatPaperExportRow));
...
function copySelectedLinks() {
    ...
    const lines = getSelectedPapersInOrder(false)
