...
"""
    ...
    print("[OK] Synced to app/docs/tables.md")
    return True
...
def update_paper_counts():
    ...
    updated = []
    for filepath in [ROOT / 'README.md', config.DOCS_DIR / 'index.md']:
...
def sync_papers_to_yaml_and_readme():
    ...
    r = subprocess.run([_sys.executable, str(config.script('sync_readme'))], cwd=str(ROOT))
    if r.returncode != 0:
...
def main():
    ...
    if not db_path.exists():
        print("[ERROR] Database not found! Run 'python app.py --init' first.")
        return False
