...
def _dataset_link_string(dataset):
    links = []
    if dataset.github_link:
        links.append(_macro_link('ghlink', dataset.github_link))
    for hf in _split_links(dataset.huggingface_link):
        links.append(_macro_link('hflink', hf))
    if dataset.website_link:
        links.append(_macro_link('bloglink', dataset.website_link))
        ...
        def write_dataset_group(header, category):
            ...
            for dataset in datasets:
                ...
                writer.writerow([
                    ...
                    _dataset_link_string(dataset)
                ])

        write_dataset_group('**Single-PL Datasets**', 'single-pl')
        write_dataset_group('**Multi-PL Datasets**', 'multi-pl')
...
def export_sft_methods_to_csv():
    ...
    _ensure_export_dir()
    csv_file = EXPORT_DIR / 'table3.csv'
    ...
    with open(csv_file, 'w', encoding='utf-8', newline='') as f:
        ...
        for method in methods:
            code = _macro_link('ghlink', method.code_link) if method.code_link else '-'
            data = _macro_link('hflink', method.data_link) if method.data_link else '-'
            model = _macro_link('hflink', method.model_link) if method.model_link else '-'
...
def export_rl_methods_to_csv():
    ...
    _ensure_export_dir()
    csv_file = EXPORT_DIR / 'table4.csv'
    ...
    with open(csv_file, 'w', encoding='utf-8', newline='') as f:
        ...
        for group_name, group_methods in size_groups.items():
            if group_methods:
                writer.writerow([f'**{group_name} Models**', '', '', '', '', '', '', '', '', ''])
...
def export_foundation_models_to_csv():
    ...
    _ensure_export_dir()
    csv_file = EXPORT_DIR / 'table5.csv'
    ...
    with open(csv_file, 'w', encoding='utf-8', newline='') as f:
        ...
        for model in models:
            code = _macro_link('ghlink', model.code_link) if model.code_link else '-'

            # Model link may be HuggingFace or Website
            model_link = '-'
            if model.model_link:
                if 'huggingface.co' in model.model_link:
                    model_link = _macro_link('hflink', model.model_link)
                elif 'modelscope' in model.model_link:
                    model_link = _macro_link('bloglink', model.model_link)
                else:
                    model_link = _macro_link('bloglink', model.model_link)
